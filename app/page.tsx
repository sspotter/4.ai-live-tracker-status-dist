    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch('/api/signals', {
          ...
          body: JSON.stringify({ category_id: activeCategory })
        });
        const data = await res.json();
        if (data.signals) {
          setSignals(data.signals);
        }
        setHasYoutubeKey(data.hasYoutubeKey);
        setHasRedditKey(data.hasRedditKey);
      } catch (e) {
      ...
      <aside className={`${sidebarOpen ? 'w-64 translate-x-0' : 'w-0 -translate-x-full md:translate-x-0'} fixed md:relative z-50 md:z-auto h-full theme-bg-secondary border-r theme-border flex flex-col flex-shrink-0 transition-all duration-300 overflow-hidden`}>
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto min-w-[256px]">
          <NavItem 
            ...
            onClick={() => { setActiveCategory(null); if (window.innerWidth < 768) setSidebarOpen(false); }}
          />
          ...
          <NavItem icon={<Github className="w-4 h-4" />} label="GitHub Signals" href="/git" />
      ...
      <main className="flex-1 overflow-y-auto relative">
        ...
        <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6 md:space-y-8">
          ...
          <header className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              <h2 className="text-2xl md:text-3xl font-semibold theme-text-primary tracking-tight">Intelligence Overview</h2>
              <p className="theme-text-muted mt-1.5 text-sm">Real-time tracking of AI tool releases and developer signals.</p>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2 border-l theme-border pl-3 ml-1">
                ...
                >
                  ...
                  <input type="file" accept=".json" className="hidden" onChange={handleImportSignals} />
                </label>
              </div>
              ...
              <div className="flex flex-col gap-8">
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <ChevronRight className={`w-4 h-4 theme-text-dim transition-transform ${!collapsedSections.dev ? 'rotate-90' : ''}`} />
                  </div>
                  {!collapsedSections.dev && (
                    displaySignals.filter(s => s.type !== 'RELEASE' && s.source !== 'YouTube').length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-6">
                        {displaySignals.filter(s => s.type !== 'RELEASE' && s.source !== 'YouTube').slice(0, 30).map((signal) => (
                          <SignalCard 
                            key={signal.id} 
                            signal={signal} 
                      ...
                      <div className="p-6 text-center theme-text-dim theme-bg-secondary rounded-xl border theme-border text-sm">
                        {loading ? "Loading signals..." : "No developer signals found."}
                      </div>
                    )
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <ChevronRight className={`w-4 h-4 theme-text-dim transition-transform ${!collapsedSections.youtube ? 'rotate-90' : ''}`} />
                  </div>
                  {!collapsedSections.youtube && (
                    displaySignals.filter(s => s.source === 'YouTube').length > 0 ? (
                      <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-6">
                        {displaySignals.filter(s => s.source === 'YouTube').slice(0, 10).map((signal) => (
                          <SignalCard 
                            key={signal.id} 
                            signal={signal} 
                            isStarred={starredIds.includes(signal.id)}
                      ...
                      <div className="p-6 text-center theme-text-dim theme-bg-secondary rounded-xl border theme-border text-sm">
                        {loading ? "Loading videos..." : "No YouTube videos found."}
                      </div>
                    )
                  )}
                </div>
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity p-4">
      <div className="theme-bg-secondary border theme-border rounded-xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors ml-2"><X className="w-5 h-5" /></button>
          </div>
        </div>
                ...
                <div className="space-y-2">
                  {subreddits.map((s: any) => (
                    <div key={s.id} className="flex items-center justify-between theme-bg-primary px-3 py-2 rounded-lg border theme-border">
                      ...
                      <button onClick={() => removeSource(s.id)} className="text-red-400 hover:text-red-300"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
    ...
    <div className="fixed inset-0 theme-overlay backdrop-blur-sm flex items-center justify-center z-50 transition-opacity p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b theme-border shrink-0">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-white transition-colors bg-[#2A2E39]/50 hover:bg-[#2A2E39] p-1.5 rounded-md">
            <X className="w-5 h-5" />
          </button>
        </div>
